...
const INVESTIGATION_CONTENT = HRS_COMPLIANCE_CONTENT.letterOfInvestigation.commercial;
const DECLARATION_CONTENT = HRS_COMPLIANCE_CONTENT.clientDeclaration.commercial;
const ELECTION_WARNING_CONTENT = HRS_COMPLIANCE_CONTENT.electionWarning.commercial;
...
function getPdfMetadata(formData) {
  ...
  return {
    productLine: 'Short-term Insurance: Commercial Lines',
...
function buildCommercialROA(pdf, formData, clientSig, advisorSig) {
  ...
  pdf.clientSummary([
    ...
    { label: 'VAT no.', value: formData.vatNo },
    { label: 'Contact person', value: formData.contactPerson },
    { label: 'Telephone', value: formData.contactNo },
    { label: 'Email', value: formData.email },
    { label: 'Business address', value: formData.riskAddress },
  ...
  // repeated in full here; this concise block records the version and acknowledgement
  // status, confirmed by the general signed ROA declaration below.
